...
GMotor rightMotor(DRIVER3WIRE, 4, 2, 3);
GMotor leftMotor(DRIVER3WIRE, 7, 5, 6);
...
Serial.begin(115200);
  ...
  rightMotor.setResolution(10);
  leftMotor.setResolution(10);

  rightMotor.setMode(AUTO);
  leftMotor.setMode(AUTO);

  rightMotor.setMinDuty(60);
  leftMotor.setMinDuty(60);
}
...
int straight = map(analogRead(A0), 0, 1023, -70, 70);
int turn = map(analogRead(A1), 0, 1023, 50, -50);
rightMotor.setSpeed(straight - turn);
leftMotor.setSpeed(straight + turn);
...
/*
//  int(compAngleY)  angle forwards and backwards (middle is  0)
float deg = 8, maxx = 150;
int value = constrain(map(compAngleY, deg, -deg, -maxx, maxx), -maxx, maxx);
